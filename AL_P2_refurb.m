function [x,xsave,err,costOrig,time] = AL_P2_refurb(ylong,A,S,R,initx,niters,lambda,mu,nx,ny,xtrue)
...
if (calc_errcost)
    ...
    costAL = calc_AL_cost(A,S,R,y,x,u,v,z,u_u,u_v,u_z,eta_u,eta_v,eta_z,lambda);
...
for ii=1:niters
    ...
    z = z_update(Q,v,x,z,u_v,u_z,R,eta_v,eta_z,eigvalsrr,nx,ny);
    eta_u = eta_u - (u - S*x);
    eta_v = eta_v - (v - R*z);
    eta_z = eta_z - (z - x);
    ...
    if (calc_errcost)
        ...
        costAL = [costAL calc_AL_cost(A,S,R,y,x,u,v,z,u_u,u_v,u_z,eta_u,eta_v,eta_z,lambda)];
...
function z = z_update(Q,v,x,z,u_v,u_z,R,eta_v,eta_z,eigvalsrr,nx,ny)
circ = 1;
...
function cost = calc_AL_cost(A,S,R,y,x,u,v,z,u_u,u_v,u_z,eta_u,eta_v,eta_z,lambda) 
