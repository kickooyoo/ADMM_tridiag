...
if do_alph
	alphas = 0:0.1:1;
	save_fname = sprintf('inpainting_mat/%s/%s/timing/inpainting_timing_%s_iters%d_wavelet%d_SNR%d_reduce%1.2d_%strue_alphw%1.1d_retunedmu%s.mat', obj, Cdiffdir, machine(1:3), niters, wavelets, SNR, reduce, true_opt, alphw, str_mod);
else
	alphas = 0.5;
	save_fname = sprintf('inpainting_mat/%s/%s/timing/inpainting_timing_%s_iters%d_wavelet%d_SNR%d_reduce%1.2d_%strue_%1.1dalph_tunedmu%s.mat', obj, Cdiffdir, machine(1:3), niters, wavelets, SNR, reduce, true_opt, alphas, str_mod);
end
...
do_exp = [0 0 0 1 1 0 1 0];
%do_exp = [0 0 0 0 0 1 0 0];
...
if do_exp(4)
	[x_MFIS, C_MFIS, time_MFIS, err_MFIS, ~] = MFISTA_inpainting_wrapper(Nx, Ny, RW, y, xinit, D, beta, round(niters), curr_folder, slice_str, 'xinf', xtrue, 'xinfnorm', norm(col(xtrue),2));
...
if do_exp(6)
	[x_SB, ~, err_SB, cost_SB, time_SB] = SB_inpainting(y, D, RWf2, xinit, niters, beta, xtrue, 'betaw', betaw, 'alphw', alphw, 'kapp', 7);
end
if do_exp(7)
	[x_P2v2, ~, err_P2v2, costOrig_P2v2, time_P2v2] = AL_P2_inpainting_ver2(y, D, RW, ...
		xinit, niters, beta, xtrue, 'alphw', alphw, 'betaw', betaw, 'inner_iter', 5, 'save_progress', [save_fname '_tmp']);
end
if exist(save_fname, 'file')
	save(save_fname, '-append')
