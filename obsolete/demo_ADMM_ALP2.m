...
%[xhat_mai, xsaved_mai, cost_mai] = SENSE_ADMM_ALP2(noisey,F,S,C1,C2,alph,beta,mu,nx,ny,img,niters);

%load ADMM_ALP2_2000iter_beta42_just_xinfs
%for iter = 1:niters
	%err(iter) = norm((abs(xsaved(:,:,iter)-x2000).^2));
%	err(iter) = norm((abs(col(xsaved_mai(:,:,iter))-img(:)).^2));
%end	


[xhat_P2,xsaved_P2,err_P2,costOrig_P2] = AL_P2_refurb(noisey,F,S,C1,C2,zeros(nx*ny,1),niters,beta,mu,nx,ny,img);

%[xhat, xsaved, cost] = SENSE_ADMM_ALP2_wavelets(noisey,F,S,C1,C2,alph,beta,mu,nx,ny,img);

%err_per_iter(:,beta_ndx,alph_ndx) = err;
%beta_xhat(:,:,:,beta_ndx,alph_ndx) = xhat_mai;

%end
%end

%save('err_per_reg_and_alph.mat','betas','alphs','err_per_iter','beta_xhat','img','noisey','F','S','C1','C2','mu');
%W = Godwt1(true(nx,ny),'level',3);
%[x, xsaved, cost] = Ramani_ISBI_ADMM(y,F,S,W,beta,mu(1),mu(2),mu(3),nx,ny,img,niters)
