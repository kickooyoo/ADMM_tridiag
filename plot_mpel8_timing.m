...
if 0
	...
	if truncate
		plot(cumsum(time_alp2t), nrmsd_alp2t, [colors(ii) '--']);
	else
		plot(cumsum([time_alp2t(1); time_alp2t]), nrmsd_alp2t, [colors(ii) '--']);
	end	
else
	plot(cumsum(time_alp2t(:,ii)), nrmsd_alp2t(:,ii), [colors(ii) '--']);
end
